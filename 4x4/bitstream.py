...
import io
import flags
from utils import reconstruct_block, decode_char_code_mapping, minimize_block, BitStreamWriter, BitStreamReader, get_espresso_cost, apply_zigzag_xor_2x2, reverse_zigzag_xor_2x2, numpy_binary_to_gray, numpy_gray_to_binary
...
class QuadTree:
    ...
    def subdivide(self, block_data):
        ...
        if self.w <= 4 and self.h <= 4:
            ...
            # Check if predictive XOR helps
            if flags.PREDICTIVE_XOR_4X4_MODE:
                xor_blk = blk.copy()
                apply_zigzag_xor_2x2(xor_blk)
                min_data_xor = minimize_block(xor_blk)
                espresso_cost_xor = get_espresso_cost(min_data_xor, use_3_bit_cube_count=True)

                if espresso_cost_xor < raw_cost and espresso_cost_xor < espresso_cost:
                    # XOR is better than raw and standard espresso
                     self.subtype = 'espresso'
                     self.minimized = min_data_xor
                     self.xor_applied = True
                     return
                
                # If XOR didn't win over raw/espresso, we might still fall back to raw or standard espresso
                # But logic continues below for standard check
            ...
            if num_cubes > 7 or raw_cost <= espresso_cost:
                # If using XOR mode, we might want to store as RAW but with XOR if that was computed? 
                # For 8x8 implementation, if raw is chosen, it stores xor_blk if cost check favored it but still failed against raw?
                # Actually in 8x8: if espresso_cost_xor < raw_cost -> espresso (xor). Else -> raw (xor).
                # Here let's follow 8x8 pattern closely if in XOR mode.
    ...
    def to_dict(self):
        if not self.children:
            ...
            if self.color is not None:
                ...
                if flags.PREDICTIVE_XOR_4X4_MODE and self.w == 4 and self.h == 4:
...
def encode_quadtree_node_bitstream(node, writer):
    if node['type'] == 'leaf':
        if node.get('subtype') in ('raw', 'espresso'):
             ...
             if node['subtype'] == 'raw':
                 writer.write_bit(1)
                 # Write XOR flag for 4x4 if applicable
                 writer.write_bit(1 if (node.get('xor_applied') and node['w'] == 4 and node['h'] == 4) else 0)
...
def decode_quadtree_node_bitstream(reader, x, y, w, h):
    ...
    if node_encoding == ENCODING_HOMOGENEOUS_0:
        ...
        # Read XOR flag for 4x4
        xor_flag = reader.read_bit() if (w == 4 and h == 4) else 0
...
def reconstruct_from_quadtree_node(node, plane_arr):
    ...
    if node['type'] == 'leaf':
        ...
        if flags.PREDICTIVE_XOR_4X4_MODE and w == 4 and h == 4 and node.get('subtype'):
            if node.get('xor_applied'):
                reverse_zigzag_xor_2x2(blk)
