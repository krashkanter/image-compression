...
class CompressionStats:
    ...
    def dump_to_file(self, filepath, compression_time, decompression_time, initial_size_bits, padded_size_bits, baseline_stats=None):
        try:
            with open(filepath, 'w') as f:
                ...
                for bit, stats in sorted(self.plane_stats.items()):
                    ...
                    if 'compressed_bits' in stats:
                        ...
                        if baseline_stats and bit in baseline_stats:
                            ...
                            diff = baseline_bits - stats['compressed_bits']
                ...
                if baseline_stats:
                    total_baseline_bits = baseline_stats.get('total', 0)
                    if total_baseline_bits > 0:
                        diff = total_baseline_bits - self.compressed_bits
                        pct_improvement = (diff / total_baseline_bits) * 100
                        f.write(f"Improvement vs 8x4 (Total, {total_baseline_bits} bits):    {diff} bits ({pct_improvement:.2f}%)\n")
                    else:
                        f.write("Improvement vs 8x4: Baseline total bits available or zero.\n")
                f.write("\n")
