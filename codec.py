from utils import (
    ...
    is_uniform_block,
    calculate_code_allotment,
    encode_cubes,
    deminimize_cubes,
    decode_cubes,
    revert_zigzag,
    calculate_decode_allotment,
)
from pla import minimize_all_blocks
import numpy as np
...
def compress_image(image, block_height, block_width):
    ...
    print("Minimizing ON-set and OFF-set with Espresso (in memory)...")
    minimized_data = minimize_all_blocks(image, block_height, block_width)
    ...
    for plane_index in range(num_planes):
        ...
        for block_index, block in enumerate(blocks):
            ...
            if is_uniform_block(block):
                ...
                block_min = minimized_data.get(plane_index, {}).get(block_index, {})
                inset_cubes = block_min.get("inset_cubes", [])
                offset_cubes = block_min.get("offset_cubes", [])
                ...
                if inset_cubes and offset_cubes:
                    ...
                    if selected_cubes_len <= 1:
                        oneCube += 1
                    elif selected_cubes_len == 2:
                        twoCubes += 1
                    elif selected_cubes_len == 3:
                        threeCubes += 1
...
def decompress(compressed_bit_stream, block_height=4, block_width=8):
    decompressed_stream = []
    for i in compressed_bit_stream:
        if i[:2] == [0, 1] or i[:2] == [0, 0]:
            ...
            original_block_bits = [int(x) for x in deminimize_cubes(decoded)]
            if offset_inset == 0:
                original_block_bits = [1 - bit for bit in original_block_bits]
...
def reconstruct_image(
    ...
    bit_planes = reconstruct_bit_planes(
        data,
